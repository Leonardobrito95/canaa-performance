      ...
      <div v-for="k in kpis" :key="k.setor" class="setor-card">
        ...
        <div class="setor-tempos-grid">
          ...
          <div class="setor-stat" title="TMA: só o tempo que o atendente humano passou de fato com o cliente. Chat e ligação juntos no valor principal.">
            ...
            <div v-if="k.volumeLigacao > 0" class="setor-stat-canal">
              <span class="canal-chip canal-chip-chat" title="Chat: tempo com atendente humano"><i class="fa-solid fa-comment"></i>{{ formatarDuracao(k.tmaMsChat) }}</span>
              <span class="canal-chip canal-chip-lig" title="Ligação: só o trecho com atendente HUMANO na linha (exclui o tempo em que a IZA/bot atendeu)"><i class="fa-solid fa-phone"></i>{{ formatarDuracao(k.tmaMsLigacao) }}</span>
              <span v-if="k.duracaoRealLigacaoMs !== null" class="canal-chip canal-chip-real" title="Duração real da ligação, do PABX (bot + humano). Costuma ser maior que o toque humano ao lado, porque a IZA/bot atende primeiro em boa parte das ligações antes de passar pro humano.">
                <i class="fa-solid fa-stopwatch"></i>{{ formatarDuracao(k.duracaoRealLigacaoMs) }}
              </span>
            </div>
          </div>
        </div>
...
.canal-chip-real { border: 1px dashed var(--warning); background: transparent; }
.canal-chip-real i { color: var(--warning); }
